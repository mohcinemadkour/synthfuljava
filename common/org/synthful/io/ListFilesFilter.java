	...
	implements FileFilter
{
    ...
    public boolean accept(File file)
    {
        if(! acceptFileName(file.getName()))
            return false;
        if(! acceptFileDate(file))
            return false;
        if(! acceptFileSize(file))
            return false;
    ...
    public boolean acceptFileName(String filename)
    {
        ...
        if (NameRegexp != null && NameRegexp.length() > 0)
            if (!filename.matches(NameRegexp))
                return false;
    ...
    private boolean acceptFileDate(Date dmodif)
    {
        ...
        if (Oldest!=null)
            if (dmodif.compareTo(Oldest)<0)
                return false;

        if (Newest!=null)
            if (dmodif.compareTo(Newest)>0)
                return false;

        return true;
    }

	// Auxiliary Accept Methods: File Size
    /**
	 * 
	 * @param file
	 *            the file
