...
public class JUtil
{
	 ...
	 */
	static public int SqlExec(Connection jconn, String sql)
	{
		try
		{
	 ...
	 * @return 1 if has results, 0 if has no results, -1 if failure.
	 */
	static public int SqlExec(JDbLogin jlogin, String sql)
	{
		try
		{
 	...
 	/**
		 ...
		 */
	static public Object SqlFetchSingleField(Connection jconn, String sql)
	{
		Object[][] oo = SqlFetch(jconn,sql);
        if (oo != null && oo.length > 0)
          if (oo[0] != null && oo[0].length > 0)
            return oo[0][0];
        return null;
	}
 	...
 	/**
		 ...
		 * @return Object of 1st row 1st column
		 */
	static public Object SqlFetchSingleField(JDbLogin jlogin, String sql)
	{
	 ...
	 */
	static public Object[][] SqlFetch(Connection jconn, String sql)
	{
	 ...
	 * @return 2D Object array
	 */
	static public Object[][] SqlFetch(JDbLogin jlogin, String sql)
	{
	 ...
	 */
	static public Object[][] SqlFetch(JDataArrayVector sqldata, String sql)
	{
		sqldata.fetch(sql);
		Object[][] results =
			new Object[sqldata.size()][];
		for (int i = 0; i < sqldata.size(); i++)
		{
			Object[] row =
				(Object[]) sqldata.get(i);
			if (row.length < 1)
				continue;
			results[i] = row;
            if (Debug)
    			System.out.println(results[i].length + ":" + results[i][0]);
		}
...
//	  System.out.println(sqldata.toString("\n", 2));
	 ...
	 * @return Sql fetch} as Object[][]
	 */
    static public Object[][] SqlFetch(JDataBufferVector sqldata, String sql)
	{
		sqldata.fetch(sql);
		Object[][] results =
			new Object[sqldata.size()][];
		for (int i = 0; i < sqldata.size(); i++)
		{
			JRowBufferVector rowvec =
				(JRowBufferVector) sqldata.get(i);
			if (rowvec.size() < 1)
				continue;
...
//	  System.out.println(sqldata.toString("\n", 2));
	 ...
	 *             the SQL exception
	 */
    static public JDataColumns getColumns(JDbLogin jlogin)
    throws SQLException
    {
	 ...
	 *             the SQL exception
	 */
    static public JDataColumns getColumns(Statement s)
    throws SQLException
    {
	 ...
	 * @throws SQLException
	 */
    static public JDataColumns getColumns(ResultSetMetaData rmd)
    throws SQLException
    {
	 ...
	 *             the SQL exception
	 */
    static public ResultSetMetaData getResultSetMetaData(Statement s)
    throws SQLException
    {
